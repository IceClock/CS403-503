...
import { SpanishInterpreter } from "./interpreter";
...
export class SpanishResolver implements ast.SyntaxVisitor<void, void> {

    private Spanishinterpreter: SpanishInterpreter;
    private scopes: ScopeStack = new ScopeStack();
    private currentFunction = FunctionType.NONE;
    private currentClass = ClassType.NONE;

    constructor(Spanishinterpreter: SpanishInterpreter) {
    ...
    resolveLocal(expr: ast.Expr, name: Token): void {
        for (let i = this.scopes.length - 1; i >= 0; i--) {
          if (name.lexeme in this.scopes[i]) {
            this.Spanishinterpreter?.['resolve'](expr, this.scopes.length - 1 - i)
            return
          }
        }
      }
